...
// Initialises the data and bar chart.
p.preload = function(){
  matrix = p.loadTable('./data/Berwick/Berwick.csv', 'csv');
  backgroundImage = p.loadImage('./data/Berwick/back.jpg');
}
...
{   
  ...
  //  p.text("parasite-parasite", buttonX + 13, buttonY + 70);
   ...
   p.line(foodChainX + 350, foodChainY - 200, foodChainX + 350, foodChainY + 200);
   p.line(foodChainX - 350, foodChainY - 200, foodChainX + 350, foodChainY - 200); 
   p.line(ecoInfoX - 10 , ecoInfoY, ecoInfoX- 10 , ecoInfoY + 100);
   p.line(ecoInfoX - 10, ecoInfoY, ecoInfoX + 800, ecoInfoY);
   p.line(ecoInfoX + 800 , ecoInfoY, ecoInfoX + 800, ecoInfoY + 100);
   p.line(ecoInfoX - 10, ecoInfoY + 100, ecoInfoX + 800, ecoInfoY + 100);
...
function drawFoodChains(index){
  ...
  for(let i = 0; i < predatorCor.length; i++){
   ...
   p.textSize(15);
   p.loadStrings("./data/Berwick/info/" + curName + ".txt", setDesc);
...
class Species {
  ...
  constructor(speciesRow, bigCircleDia, idx, total, columnNum){
    ...
    for(let i = 0; i < columnNum ; i++){
      ...
      this.img = p.loadImage("./data/Berwick/pics/" + this.name + ".jpg");
