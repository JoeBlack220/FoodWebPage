...
p.preload = function() {
  matrix = p.loadTable('./data/BajaCalifornia/BajaCalifornia3D.csv', 'csv');
}
...
p.setup = function(){
  ...
  for (let r=1; r<matrix.getRowCount(); r++){
    let relations = [];
    for (let c=4; c<matrix.getColumnCount(); c++){
...
p.draw = function()
{
 ...
 // directionalLight(150, 150, 150, 1, -0.5, 0.5);
 // directionalLight(150, 150, 150, 1, 0, 0.5);
 
 // add camera interaction
  var minDist = 200.0;
  var maxDist = 1500.0;
  //  Zoom in/out with the camera
  var cameraDistance = p.lerp(minDist, maxDist, p.float(p.mouseY * 2) / p.height);
  ...
  var s = p.second();
  var t = (p.int(s/2))%13+1;
  for (let i=1; i<13; i++){
    var c = getColor(i);
    p.stroke(c);
    p.noFill();
    p.push();
    p.translate(0, 0, i*100-500);
    p.circle(0, 0, 700);
    if (t==i){
      p.fill(c, 10);
      p.circle(0, 0, 700);
...
function getColor(level){
  switch (level){
    ...
    case 5:
      return p.color(190,186,218);
    case 6:
      return p.color(177,89,40);
    case 7:
      return p.color(141,211,199);
    case 8:
      return p.color(51,160,44);
    case 9:
      return p.color(31,120,180);
    case 10:
      return p.color(227,26,28);
    case 11:
      return p.color(106,61,154);
    case 12:
      return p.color(255,255,179);
    }
}
